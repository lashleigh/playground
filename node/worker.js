...
console.log((new Date() - start_time)/1000.0)
print_coords(grid)
...
function mass_at_radius(r) {
  var num = 0;
  r = Math.floor(r);
  for(var i=-r; i<r; i++) {
    for(var j=-r; j < r; j++) {
      if(i*i+j*j <= r*r) { 
        num += grid[i+data.radius][j+data.radius];
      } 
    }
  }
  return num
}
function dimension(grid) {
  var masses = []
  for(var i=1; i< data.radius/2; i++) {
    var m = mass_at_radius(i);
    console.log(i+" "+m);
    masses.push(m);
  }
  return masses;
}
function print_coords(grid) {
  console.log("COORDS");
